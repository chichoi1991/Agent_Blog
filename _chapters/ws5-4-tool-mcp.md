...
이번 실습에서는 에이전트가 사용자를 대신하여 스펙시트의 내용을 메일로 발송해주기 위한 MCP 도구를 추가해보도록 합니다.
...
테스트 프롬프트
```
식기세척기 제품군들에 대해 뉴스레터를 만들어서 나에게 메일로 보내줘
```
...
다음은 일반 커넥터를 이용한 메일/팀즈 게시물 도구를 추가해 보도록 합니다.

---

← [이전: Step 3. 참조자료 추가]({{ '/chapters/ws5-3-knowledge/' | relative_url }}) | [다음: Step 5. 커넥터(메일 발송) 추가]({{ '/chapters/ws5-5-tool-connector/' | relative_url }}) →
