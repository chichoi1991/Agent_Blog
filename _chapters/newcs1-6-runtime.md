...
> **짚고 가기:** 아래 컨테이너 내부 구조(엔진 코드네임·OS 빌드 등)는 **런타임 관찰(runtime inspection) 결과**다. 외부 공개 자료에서는 엔진 코드네임을 "AI 엔진"으로 추상화하고 내부 경로·빌드 정보는 빼는 것을 권장한다.
...
| **/dracarys/** | AI 에이전트 엔진 본체 — sandbox 권한으로 접근 차단 🔒 |
...
- **엔진 본체(`/dracarys/`)는 에이전트조차 못 읽는다.** 프롬프트 인젝션으로 에이전트가 탈취돼도 엔진·다른 세션엔 닿지 못함.
...
→ 클래식이 *검색 스니펫*만으로 답했다면, New CLI 에이전트는 **원본 파일 자체를 열어** 수만 행 엑셀·수백 쪽 PDF를 코드로 집계·변환합니다. 2장의 *대용량 파일 +8.3점 / 코드 인터프리터 +41.8점*(NDA)이 **왜** 가능한지가 여기서 드러납니다.
