...
> 중요! 쉐어포인트 참조는 파일을 Microsoft search 를 기반으로 문서의 내용을 검색합니다. 따라서 텍스트 중심의 자료 조회에는 효과적이지만, 엑셀의 데이터 분석이 필요한 경우, LLM과 RAG의 구조적 한계로 정확한 답변을 하지 못합니다.

따라서, Excel의 데이터를 참조하여 답변을 만들어야 하는 경우 워크샵 마지막의 Excel 데이터 참조 세션을 참고합니다.
...
← [이전: Step 2. 기본 지침 설정]({{ '/chapters/ws5-2-instructions/' | relative_url }}) | [다음: Step 4. MCP 도구 추가]({{ '/chapters/ws5-4-tool-mcp/' | relative_url }}) →
