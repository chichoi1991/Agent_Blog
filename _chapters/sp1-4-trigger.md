...
Part 1에서 작성한 지침에 **섹션 6**을 추가합니다. 에이전트 설정 → **지침** 에서 기존 시나리오 G 이후, `# 응답 원칙` 앞에 아래 내용을 삽입합니다:
...
> **핵심 포인트:** 2단계에서 **Teams 논의와 이전 메일 스레드를 먼저 검색**하여 내부 맥락을 확보한 후 경제지표를 조회합니다. 에이전트가 "우리 팀에서 이미 논의된 맥락"을 반영한 판단을 제공합니다.
