...
> 📖 **참조**: [Generative orchestration guidance](https://learn.microsoft.com/en-us/microsoft-copilot-studio/guidance/generative-mode-guidance)

<div class="info-box tip">
<strong>💡 실무 팁 — "무엇을 할 수 있는지"를 지침에 명시하세요</strong>
에이전트가 <strong>사용할 수 있는 도구 목록</strong>을 지침에 적어주면, LLM이 더 자연스럽게 질문을 이어가고 적절한 Action을 선택합니다. "알아서 해"보다 <strong>"이 도구들로 이런 범위 안에서 해"</strong>가 훨씬 좋은 결과를 만듭니다.
</div>
...
> 📖 **참조**: [Create and edit topics](https://learn.microsoft.com/en-us/microsoft-copilot-studio/authoring-create-edit-topics)

<div class="info-box warning">
<strong>⚠️ 가장 빈번한 실수 — Topic 마지막에 메시지 노드 넣기</strong>
Generative Orchestration 모드에서 Topic 마지막에 고정 메시지를 넣으면, LLM이 <strong>해당 Topic 이후의 판단을 하지 못합니다</strong>. 대신 <strong>출력 변수(Output Variable)</strong>로만 결과를 반환하고, 최종 답변은 에이전트가 자율적으로 생성하도록 위임하세요.
</div>
...
> 📖 **참조**: [Work with variables](https://learn.microsoft.com/en-us/microsoft-copilot-studio/authoring-variables) · [Variables overview](https://learn.microsoft.com/en-us/microsoft-copilot-studio/authoring-variables-about)
...
> 📖 **참조**: [System variables list](https://learn.microsoft.com/en-us/microsoft-copilot-studio/authoring-variables-about#system-variables)
...
Flow Input에서 System 변수를 <strong>바로 참조하면 런타임에 값이 비어 있습니다</strong>. 반드시 Topic 변수에 먼저 저장한 후, 그 변수를 Flow Input에 매핑하세요.
</div>
...
> 📖 **참조**: [Pass variables between topics](https://learn.microsoft.com/en-us/microsoft-copilot-studio/authoring-variables#pass-variables-between-topics)
...
> 📖 **참조**: [Set global variables from external sources](https://learn.microsoft.com/en-us/microsoft-copilot-studio/authoring-variables-bot#set-global-variables-from-external-sources)
...
> 📖 **참조**: [Create expressions using Power Fx](https://learn.microsoft.com/en-us/microsoft-copilot-studio/advanced-power-fx)
...
| 방식 | 적합한 경우 | 특징 | 참조 |
|---|---|---|---|
| **커넥터 기반** | Outlook, Teams 등 표준 서비스 | 설정만으로 즉시 사용 | [Learn](https://learn.microsoft.com/en-us/microsoft-copilot-studio/advanced-plugin-actions) |
| **Power Automate 플로우** | 복잡한 다단계 처리 | 오류 처리, 루프 지원 | [Learn](https://learn.microsoft.com/en-us/microsoft-copilot-studio/advanced-flow) |
| **MCP 서버** | 자체 개발 도구/서비스 | 유연한 외부 도구 연동 | |
| **AI Prompt** | 맞춤형 AI 응답 생성 | 프롬프트 완전 제어 | [Learn](https://learn.microsoft.com/en-us/microsoft-copilot-studio/prompts-overview) |

### Power Automate Flow 설계 실전 팁
...
<div class="info-box warning">
<strong>⚠️ 실패 패턴 — Flow 안에서 모든 로직 처리</strong>
Flow 안에서 if/else + 텍스트 조합까지 모두 처리하면 <strong>디버깅 지옥, 유지보수 불가</strong> 상태에 빠집니다. Flow는 <strong>"명령을 받아 실행하고 결과만 돌려주는"</strong> 역할에 집중하세요.
</div>
...
> 📖 **참조**: [Use actions with custom agents](https://learn.microsoft.com/en-us/microsoft-copilot-studio/advanced-plugin-actions)

---

## Generative Orchestration 실전 가이드
...
<div class="info-box tip">
<strong>💡 테스트 패널 vs 실제 동작의 차이</strong>
테스트 패널은 실제 채널(Teams, Web)과 <strong>동작이 다를 수 있습니다</strong>. 특히 인증 관련 Action, 채널별 Adaptive Card 렌더링, Fallback 동작에서 차이가 발생합니다. 반드시 <strong>게시(Publish) 후 실제 채널에서도 테스트</strong>하세요.
</div>

---

## 워크샵에서 가장 많이 나오는 실수 TOP 7
...
이번 챕터에서 Agent를 만드는 핵심 방법론을 익혔습니다. 다음 <strong>Chapter 4</strong>에서는 만들어진 Agent를 외부 시스템(Power Automate, API, MCP)과 연동하는 구체적인 방법과 인증 설정을 다룹니다.
</div>
