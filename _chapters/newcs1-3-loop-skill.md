...
**▶ 포인트** — 하네스의 심장은 "에이전틱 루프"다. 스킬·툴·메모리·서브에이전트는 전부 이 루프가 소비하는 재료일 뿐이다. New 오케스트레이터는 이 루프를 **(1) 도구 선택 정확도 (2) 재귀 실행 (3) 실패 적응 (4) 자기 검증 분리**로 강화했다.
</div>
...
> **짚고 가기:** 에이전트가 무너지는 데는 딝 두 가지 이유가 있고, 새 코어는 그 둘을 정조준했다.
...
**실패 모드 2 — 자기 평가의 관대함**
모델은 자기 결과물을 평가하라고 하면 거의 항상 과대 칭찬합니다. 해법은 **작업자(generator)와 평가자(evaluator)를 분리**하는 것. Anthropic은 GAN에서 착안해 generator↔evaluator 루프를 만들었고, "자기 일을 비판하게 만드는 것보다 별도 평가자를 회의적으로 튜닝하는 게 훨씬 쉽다"고 결론냈습니다.
...
<div class="info-box note" markdown="1">
**▶ 포인트** — 스킬은 "X를 어떻게 하는지"를 markdown으로 한 번 적어두고 필요할 때만 불러 쓰는 재사용 지침이다. 이건 단순 편의 기능이 아니라 Anthropic이 말하는 **"just-in-time 컨텍스트"의 엔터프라이즈 구현**이며, 외부(GitHub Copilot·Claude) 스킬까지 그대로 가져올 수 있는 업계 호환 자산이다.
</div>
...
- **Just-in-time 검색** — 모든 데이터를 미리 욱여넣지 말고, 경량 식별자(파일 경로·쿼리·링크)만 들고 있다가 런타임에 필요한 것만 로드. Claude Code의 `CLAUDE.md` + `glob`/`grep`가 정확히 이 패턴.
...
참고로 Anthropic은 frontend-design 같은 스킬을 공개 `SKILL.md`로 배포합니다. New Copilot Studio가 이런 스킬을 import한다는 건, **Microsoft 울타리 밖 생태계 자산을 직접 끌어 쓴다**는 의미입니다.

### 3.4 스킬 로드맵 (NDA — 날짜 취급 주의)
