...
short_title: "Connect a Snowflake MCP server"
description: "An end-to-end guide to connecting a Snowflake-managed MCP server to a Copilot Studio agent using delegated user OAuth through Microsoft Entra ID."
order: 3
category: "catblog"
...
**Translated article** — This article is based on [Wiring up a Snowflake-managed MCP server in Copilot Studio](https://microsoft.github.io/mcscatblog/posts/snowflake-mcp-copilot-studio/) by hasharaf, missbets on [The Custom Engine](https://microsoft.github.io/mcscatblog/) (2026-05-22). The original wording takes precedence.
</div>

<figure class="screenshot">
  <img src="{{ '/assets/catblog/snowflake-mcp-copilot-studio/header.png' | relative_url }}" alt="A Copilot Studio agent connected to Snowflake through an MCP link" loading="lazy" onerror="this.style.display='none';this.parentNode.classList.add('pending')">
</figure>
...
<figure class="screenshot">
  <img src="{{ '/assets/catblog/snowflake-mcp-copilot-studio/01-sample-data-result.png' | relative_url }}" alt="Snowsight results pane showing the sample data script completed successfully" loading="lazy" onerror="this.style.display='none';this.parentNode.classList.add('pending')">
  <figcaption>Two filled tables is all we need before turning on Cortex Search.</figcaption>
</figure>
...
<figure class="screenshot">
  <img src="{{ '/assets/catblog/snowflake-mcp-copilot-studio/02-describe-mcp-server.png' | relative_url }}" alt="DESCRIBE MCP SERVER output listing the bundled tool specification" loading="lazy" onerror="this.style.display='none';this.parentNode.classList.add('pending')">
  <figcaption>The DESCRIBE output is our contract with the LLM. The name and description fields are exactly what the agent’s model sees when it decides whether to call a tool, so keep them in snake_case and make the descriptions precise.</figcaption>
</figure>
...
<figure class="screenshot">
  <img src="{{ '/assets/catblog/snowflake-mcp-copilot-studio/03-grant-usage-role.png' | relative_url }}" alt="Snowsight results pane confirming role and USAGE grants were applied" loading="lazy" onerror="this.style.display='none';this.parentNode.classList.add('pending')">
  <figcaption>Six GRANT USAGE lines and a warehouse grant are enough for the read-only agent use case.</figcaption>
</figure>
...
<figure class="screenshot">
  <img src="{{ '/assets/catblog/snowflake-mcp-copilot-studio/04-create-delegate-user.png' | relative_url }}" alt="Snowsight worksheet showing delegate user creation and role binding completed" loading="lazy" onerror="this.style.display='none';this.parentNode.classList.add('pending')">
  <figcaption>Creating the delegate user and binding the role in a single Snowsight worksheet.</figcaption>
</figure>
...
<figure class="screenshot">
  <img src="{{ '/assets/catblog/snowflake-mcp-copilot-studio/05-show-grants-user.png' | relative_url }}" alt="SHOW GRANTS TO USER output listing the SALESPROFESSIONAL role for the delegate user" loading="lazy" onerror="this.style.display='none';this.parentNode.classList.add('pending')">
  <figcaption>If the role is not on this list, the OAuth handshake will succeed and the tool call will still fail with “insufficient privileges.”</figcaption>
</figure>

<figure class="screenshot">
  <img src="{{ '/assets/catblog/snowflake-mcp-copilot-studio/06-show-grants-role.png' | relative_url }}" alt="SHOW GRANTS TO ROLE output listing all USAGE grants for the SALESPROFESSIONAL role" loading="lazy" onerror="this.style.display='none';this.parentNode.classList.add('pending')">
  <figcaption>Confirm the role itself can see the database, schema, Cortex Search services, MCP server, and warehouse before moving on.</figcaption>
</figure>
...
<figure class="screenshot">
  <img src="{{ '/assets/catblog/snowflake-mcp-copilot-studio/09-resource-app-expose-api.png' | relative_url }}" alt="Expose an API blade for the Snowflake OAuth Resource app registration showing the Application ID URI and session:role-any scope" loading="lazy" onerror="this.style.display='none';this.parentNode.classList.add('pending')">
  <figcaption>This is the audience the access token will carry. It has to match EXTERNAL_OAUTH_AUDIENCE_LIST in the Snowflake integration exactly.</figcaption>
</figure>
...
<figure class="screenshot">
  <img src="{{ '/assets/catblog/snowflake-mcp-copilot-studio/07-client-app-overview.png' | relative_url }}" alt="Client app registration Overview blade showing the Application (client) ID and tenant" loading="lazy" onerror="this.style.display='none';this.parentNode.classList.add('pending')">
  <figcaption>Copy the Application (client) ID and Directory (tenant) ID from this Overview tab. You will paste both into the Copilot Studio MCP form in Step 6.</figcaption>
</figure>

<figure class="screenshot">
  <img src="{{ '/assets/catblog/snowflake-mcp-copilot-studio/08-client-app-api-permissions.png' | relative_url }}" alt="Client app API permissions blade showing the session:role-any delegated permission with admin consent granted" loading="lazy" onerror="this.style.display='none';this.parentNode.classList.add('pending')">
  <figcaption>Without admin consent, the first user sign-in fails with the generic “needs admin approval” page.</figcaption>
</figure>
...
<figure class="screenshot">
  <img src="{{ '/assets/catblog/snowflake-mcp-copilot-studio/10-describe-integration.png' | relative_url }}" alt="DESCRIBE INTEGRATION output with ENABLED = true, Azure issuer URL, JWKS URL, and audience list configured" loading="lazy" onerror="this.style.display='none';this.parentNode.classList.add('pending')">
  <figcaption>ENABLED = true and a matching audience are the only two fields we check before moving on.</figcaption>
</figure>
...
<figure class="screenshot">
  <img src="{{ '/assets/catblog/snowflake-mcp-copilot-studio/11-cs-create-mcp-form.png' | relative_url }}" alt="Copilot Studio Add new MCP form filled in with OAuth 2.0 selected" loading="lazy" onerror="this.style.display='none';this.parentNode.classList.add('pending')">
  <figcaption>The Server URL is the only field most people get wrong. Use the exact pattern from the table above, including the `/api/v2/databases/.../mcp-servers/&lt;MCP_SERVER_NAME&gt;` suffix.</figcaption>
</figure>
...
1. Open [Power Apps](https://make.powerapps.com), then use the environment switcher in the upper-right to switch to the **same environment** you used in Copilot Studio.
...
<figure class="screenshot">
  <img src="{{ '/assets/catblog/snowflake-mcp-copilot-studio/12a-powerapps-custom-connectors.png' | relative_url }}" alt="Power Apps custom connectors list showing the auto-generated Snowflake MCP connector" loading="lazy" onerror="this.style.display='none';this.parentNode.classList.add('pending')">
  <figcaption>The connector appears in Power Apps under More &gt; Custom connectors, in the same environment as your agent.</figcaption>
</figure>

<figure class="screenshot">
  <img src="{{ '/assets/catblog/snowflake-mcp-copilot-studio/12b-connector-security-redirect-url.png' | relative_url }}" alt="Custom connector with the OAuth 2.0 redirect URL shown at the bottom of the Security tab" loading="lazy" onerror="this.style.display='none';this.parentNode.classList.add('pending')">
  <figcaption>The Redirect URL field is at the bottom of the Security tab. Copy it — you will paste it into the client app’s Authentication blade in the next step.</figcaption>
</figure>
...
Open the **Client app** registration, go to **Authentication**, choose **Add a platform > Web**, paste the redirect URL from the previous step, then click **Configure**. You should see *“<client app name> was successfully updated.”*

<figure class="screenshot">
  <img src="{{ '/assets/catblog/snowflake-mcp-copilot-studio/16-azure-add-redirect-uri.png' | relative_url }}" alt="Client app registration Authentication blade with the Power Platform redirect URI added under Web platform" loading="lazy" onerror="this.style.display='none';this.parentNode.classList.add('pending')">
  <figcaption>Add the redirect URI under the Web platform, not SPA or public client. Anything else fails silently at token exchange.</figcaption>
</figure>
...
<figure class="screenshot">
  <img src="{{ '/assets/catblog/snowflake-mcp-copilot-studio/18-cs-add-tool-filter-mcp.png' | relative_url }}" alt="Copilot Studio Add tool dialog filtered to MCP" loading="lazy" onerror="this.style.display='none';this.parentNode.classList.add('pending')">
  <figcaption>Filter the tool picker by Model Context Protocol to find your newly created MCP tool quickly.</figcaption>
</figure>

<figure class="screenshot">
  <img src="{{ '/assets/catblog/snowflake-mcp-copilot-studio/19-cs-add-tool-picker.png' | relative_url }}" alt="Copilot Studio tool picker showing discovered customer_search and product_search tools" loading="lazy" onerror="this.style.display='none';this.parentNode.classList.add('pending')">
  <figcaption>If the tool list is empty here, discovery failed. Re-check the Server URL and the connection status before going any further.</figcaption>
</figure>

<figure class="screenshot">
  <img src="{{ '/assets/catblog/snowflake-mcp-copilot-studio/20-cs-tools-in-agent.png' | relative_url }}" alt="Copilot Studio agent Tools blade listing the Snowflake MCP tools" loading="lazy" onerror="this.style.display='none';this.parentNode.classList.add('pending')">
  <figcaption>Once tools show up here, the Snowflake side is correctly wired.</figcaption>
</figure>
...
<figure class="screenshot">
  <img src="{{ '/assets/catblog/snowflake-mcp-copilot-studio/21-cs-agent-test.png' | relative_url }}" alt="Copilot Studio test pane showing the agent answering after invoking Snowflake MCP tools" loading="lazy" onerror="this.style.display='none';this.parentNode.classList.add('pending')">
  <figcaption>The grounded answer plus the Activity map showing a tool call is what you want to see. A plain LLM answer with no tool activity means the agent did not actually hit Snowflake.</figcaption>
</figure>
...
<figure class="screenshot">
  <img src="{{ '/assets/catblog/snowflake-mcp-copilot-studio/22-cs-agent-instructions.png' | relative_url }}" alt="Copilot Studio agent Overview tab showing instructions to use the Snowflake MCP tools" loading="lazy" onerror="this.style.display='none';this.parentNode.classList.add('pending')">
  <figcaption>One explicit sentence in the instructions is usually enough to stop the model from answering from general knowledge.</figcaption>
</figure>

You can also disable **Web search** and **Use general knowledge** in the agent’s generative settings if you want to be strict.

<figure class="screenshot">
  <img src="{{ '/assets/catblog/snowflake-mcp-copilot-studio/23-cs-agent-knowledge-settings.png' | relative_url }}" alt="Copilot Studio agent generative settings with Web search and Use general knowledge turned off" loading="lazy" onerror="this.style.display='none';this.parentNode.classList.add('pending')">
  <figcaption>Belt-and-braces: turning these off forces the agent to ground every answer in its tools.</figcaption>
</figure>
...
The most useful diagnostic is the agent’s **Activity** tab. Click the test conversation, then click the tool node (for example `customer_search`). The right-hand pane shows:
...
| --- | --- | --- |
| `MCP Server tool error: No tool result received calling Cortex Agent` | Cortex Agent disabled on this Snowflake account (often a trial). | Enable Cortex or use a paid account. See the prerequisite section. |
...
<figure class="screenshot">
  <img src="{{ '/assets/catblog/snowflake-mcp-copilot-studio/13-custom-connector-general.png' | relative_url }}" alt="Power Apps custom connector General tab showing the connector scheme and host pointing to the Snowflake account" loading="lazy" onerror="this.style.display='none';this.parentNode.classList.add('pending')">
  <figcaption>The auto-generated connector uses HTTPS and the Snowflake account host. Leave the scheme alone; only the host needs a sanity check.</figcaption>
</figure>

<figure class="screenshot">
  <img src="{{ '/assets/catblog/snowflake-mcp-copilot-studio/14-custom-connector-host.png' | relative_url }}" alt="Power Apps custom connector General tab with the Snowflake account host filled in" loading="lazy" onerror="this.style.display='none';this.parentNode.classList.add('pending')">
  <figcaption>Confirm the host matches `&lt;orgname&gt;-&lt;accountname&gt;.snowflakecomputing.com` exactly — no trailing path, no protocol.</figcaption>
</figure>

<figure class="screenshot">
  <img src="{{ '/assets/catblog/snowflake-mcp-copilot-studio/15-custom-connector-security.png' | relative_url }}" alt="Power Apps custom connector Security tab with OAuth 2.0 configured for Azure AD and showing client ID, secret, and resource URI" loading="lazy" onerror="this.style.display='none';this.parentNode.classList.add('pending')">
  <figcaption>The Resource URL must match api://&lt;RESOURCE_APP_CLIENT_ID&gt; and the scope must be session:role-any. A wrong audience here is the most common cause of token rejection.</figcaption>
</figure>

<figure class="screenshot">
  <img src="{{ '/assets/catblog/snowflake-mcp-copilot-studio/17-custom-connector-test.png' | relative_url }}" alt="Power Apps custom connector Test tab showing a successful server call" loading="lazy" onerror="this.style.display='none';this.parentNode.classList.add('pending')">
  <figcaption>A green “Invoke server” response means the OAuth handshake completed and the bearer token reached the MCP endpoint.</figcaption>
</figure>

<figure class="screenshot">
  <img src="{{ '/assets/catblog/snowflake-mcp-copilot-studio/24-troubleshooting-test-section.png' | relative_url }}" alt="Power Apps custom connector Test tab showing the expected schema validation error response" loading="lazy" onerror="this.style.display='none';this.parentNode.classList.add('pending')">
  <figcaption>The 405 + schema validation warning is expected for MCP-backed connectors. Ignore it as long as the connection itself shows as connected.</figcaption>
</figure>
