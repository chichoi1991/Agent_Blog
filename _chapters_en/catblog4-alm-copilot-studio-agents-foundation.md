...
short_title: "Copilot Studio agent ALM fundamentals"
description: "The core of application lifecycle management (ALM) for Copilot Studio agents: a safe baseline built on environments, solutions, publishers, and pipelines."
order: 4
category: "catblog"
...
**Translated article** — This article is based on [ALM for Copilot Studio Agents: The Foundation](https://microsoft.github.io/mcscatblog/posts/alm-copilot-studio-agents-foundation/) by jpapadimitriou on [The Custom Engine](https://microsoft.github.io/mcscatblog/) (2026-06-03). The original wording takes precedence.
</div>

<figure class="screenshot">
  <img src="{{ '/assets/catblog/alm-copilot-studio-agents-foundation/header.png' | relative_url }}" alt="ALM foundations for Copilot Studio agents" loading="lazy" onerror="this.style.display='none';this.parentNode.classList.add('pending')">
</figure>
...
|---|---|---|
| Development | Unmanaged solution (active work) | Daily authoring |
...
<figure class="screenshot">
  <img src="{{ '/assets/catblog/alm-copilot-studio-agents-foundation/alm-solution-evolution-diagram.png' | relative_url }}" alt="Diagram explaining the Power Platform managed solution deployment model" loading="lazy" onerror="this.style.display='none';this.parentNode.classList.add('pending')">
  <figcaption>In Dev, components like agents, workflows, and variables live inside an unmanaged solution where makers freely edit them. The solution is exported as a versioned managed package and deployed to Test and then Production. Changes flow in one direction only: from Dev, through the package, to downstream environments.</figcaption>
</figure>
...
<figure class="screenshot">
  <img src="{{ '/assets/catblog/alm-copilot-studio-agents-foundation/alm_solution_pipeline_flow.png' | relative_url }}" alt="Diagram showing the Power Platform pipeline automated deployment process" loading="lazy" onerror="this.style.display='none';this.parentNode.classList.add('pending')">
  <figcaption>A solution in Dev is exported as a managed package and fed into a pipeline, which deploys it to Test and Production. The pipeline resolves per-environment configuration during deployment and records each deployment in its history.</figcaption>
</figure>
...
You don’t have to do this by hand. The [agentic improvement loop](https://microsoft.github.io/mcscatblog/posts/agentic-improvement-loop/) shows how to iterate on quality, and [automated evaluations in Azure DevOps](https://microsoft.github.io/mcscatblog/posts/copilot-studio-eval-gate-azure-devops/) shows how to turn evals into an enforced gate that blocks a change when quality drops. Wiring evals into your promotion flow is the subject of the automation post in this series.
