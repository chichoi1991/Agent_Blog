...
short_title: "Agent authentication admin control"
description: "How the Power Platform admin control 'Require Microsoft authentication for agents' solves the identity boundary problem for internal employee-facing agents that DLP channel blocking could not address."
order: 2
category: "catblog"
...
**Translated article** — This article is based on [The Admin Control That Closes the Door Before "Hello"](https://microsoft.github.io/mcscatblog/posts/agent-authentication-controls/) by adilei on [The Custom Engine](https://microsoft.github.io/mcscatblog/) (2026-06-14). The original wording takes precedence.
</div>

<figure class="screenshot">
  <img src="{{ '/assets/catblog/agent-authentication-controls/header.png' | relative_url }}" alt="Sometimes even hello is too much." loading="lazy" onerror="this.style.display='none';this.parentNode.classList.add('pending')">
</figure>
...
<figure class="screenshot">
  <img src="{{ '/assets/catblog/agent-authentication-controls/on-sign-in-message.png' | relative_url }}" alt="Message node in the Copilot Studio On Sign In topic with maker-editable text" loading="lazy" onerror="this.style.display='none';this.parentNode.classList.add('pending')">
  <figcaption>The On Sign In topic. That greeting is authored by the maker and can be shown before the user has authenticated.</figcaption>
</figure>
...
<figure class="screenshot">
  <img src="{{ '/assets/catblog/agent-authentication-controls/authentication-for-agents-pane.png' | relative_url }}" alt="Authentication for agents pane in the Power Platform admin center with Require Microsoft authentication selected" loading="lazy" onerror="this.style.display='none';this.parentNode.classList.add('pending')">
  <figcaption>Under Security &gt; Identity and access. “Require Microsoft authentication” is the new, strictest user-facing posture.</figcaption>
</figure>
...
<figure class="screenshot">
  <img src="{{ '/assets/catblog/agent-authentication-controls/agent-details-authentication.png' | relative_url }}" alt="Agent Details table in Copilot Studio Kit showing Custom Entra ID in the End User Authentication Type column" loading="lazy" onerror="this.style.display='none';this.parentNode.classList.add('pending')">
  <figcaption>The Copilot Studio Kit inventory records each agent’s authentication type. Anything other than `Integrated` is an agent that will be blocked once you enforce Require Microsoft authentication.</figcaption>
</figure>
...
For a fuller tour of what the Kit’s inventory and governance modules can do, see [Copilot Studio Kit: Beyond Test Automation](https://microsoft.github.io/mcscatblog/posts/copilot-studio-kit/).
...
- **Default posture:** for environments that host only employee-facing agents (don’t mix scenarios), make **Require Microsoft authentication** the default. Custom web UI through the M365 Agents SDK client is fully supported.
